...
#     fcar
    ...
    #bargraph
    plt.figure(figsize=(10, 6),facecolor=(16/255, 30/255, 39/255))
    # # Set the background color of the plot
    plt.rcParams['axes.facecolor'] = (16/255, 30/255, 39/255)

    # # Define custom colors for the bars
    colors = ['#2D9596', '#F1FADA', '#2D9596', '#F1FADA']
    plt.bar(mean_row.columns, mean_row.values[0], color=colors)
    plt.xlabel('Course Outcome', color='white')
    plt.ylabel('Average Attainment', color='white')
    plt.title('Average percentage Attainment for Each Course Outcome', color='white')
    plt.xticks(rotation=45, color='white')  # Rotate x-axis labels for better readability
    plt.yticks(color='white')
    plt.ylim(0, 100)  # Setting y-axis limits to 0% and 100%
    plt.tight_layout()
    ...
    # bars = plt.bar(mean_row['Course Outcome'], mean_row['Average Attainment'], color=colors)

    # plt.xlabel('Course Outcome')
    # plt.ylabel('Average Attainment')
    # plt.title('Average Attainment for Each Course Outcome')
    ...
    # plt.pie(mean_row.values[0], labels=mean_row.columns, autopct='%1.1f%%', startangle=140)
    # plt.title('Average Attainment Distribution for Each Course Outcome')
    # plt.tight_layout()
    ...
    return render_template('result.html', result=fcar_html_table, plot=graph)
